...
--EMPLOYEE 테이블과 DEPARTMENT 테ㅣ블을 연결 할 때 
--DEPT_CODE 카럶과 DEPT_ID이 같은 값을 가지고 있음으로 
--해당 칼럼들을 기준으로 옴긴다.
...
WHERE DEPT_CODE = DEPT_ID; --WHERE절에 같은 값을 가지고 있는 칼럼을 작성
...
--내부조인에 문제점
--연결에 사용되는 칼럼에 값이 NULL이면 조회 결과에 포함되지 않는다. 
...
--D왼쪽 테이블 컬럼과 오른쪽 테이블 컬럼과 같은 값이 없으면
--강제로 추가
...
WHERE DEPT_CODE = DEPT_ID --EMP+DEP 조인
AND LOCATION_ID = LOCAL_CODE; --앞에서 두개 조인하고 LOC 조인
...
-- ANSI
SELECT EMP_NAME,JOB_NAME,DEPT_TITLE,LOCAL_NAME,SALARY
FROM EMPLOYEE E
JOIN JOB J USING(JOB_CODE)
JOIN DEPRATMENT (DEPT_CODE = DEPT_ID);
...
ORDER BY DEPT_TITLE,E.JOB_CODE DESC;
...
--연봉에 보너스포인트를 적용하시오.
SELECT EMP_NAME,JOB_NAME,SALARY"월급",((SALARY*12)*NVL2(BONUS,BONUS+1,1)) AS "연봉"
FROM EMPLOYEE
...
SELECT EMP_NAME,DEPT_TITLE,LOCAL_NAME,N.NATIONAL_NAME
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN LOCATION L ON(LOCATION_ID=LOCAL_CODE)
JOIN NATIONAL N ON(L.NATIONAL_CODE=N.NATIONAL_CODE)
WHERE L.NATIONAL_CODE IN('KO','JP')
ORDER BY LOCAL_NAME,DEPT_CODE;


-- 8. 같은 부서에 근무하는 직원들의 사원명, 부서코드, 동료이름을 조회하시오.
-- SELF JOIN 사용
SELECT M.EMP_NAME,M.DEPT_CODE,N.EMP_NAME
FROM EMPLOYEE M
JOIN EMPLOYEE N ON(M.DEPT_CODE=N.DEPT_CODE)
WHERE N.EMP_NAME != M.EMP_NAME
ORDER BY M.EMP_NAME;
