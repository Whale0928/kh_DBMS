...
SELECT DEPT_CODE, SALARY SUM(SALARY) --23행
FROM EMPLOYEE;
--> DEPT_CODE컬럼은 23행 SUM(SALARY)컬럼은 1행으로
--> 하나의 표에 작성할 수 없다
...
SELECT DEPT_CODE , FLOOR(AVG(SALARY))
FROM EMPLOYEE
WHERE SALARY >= 3000000 --급여가 300만이상인 직원의 평균급여.
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE;
