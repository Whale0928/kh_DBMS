...
-- MEMBER 테이블 생성
CREATE TABLE MEMBER(
    MEMBER_ID VARCHAR2(20),  --가변 길이 문자열 20BYTE (영어,숫자만 작성 시 20글자)
    MEMBER_PWD VARCHAR2(20),
    MEMBER_NAME VARCHAR2(30), 
    MEMBER_SSN CHAR(14),  --999999-9999999
    ENROLL_DATE DATE DEFAULT SYSDATE
);


--DEFAULT : 입력되는 값이 없거나 DEFAULT 키워드 사용 시에 기록 되어질 값을 지정


-- 만든 테이블 확인
SELECT * FROM MEMBER;
SELECT * FROM USER_TABLES; --데이터 딕셔너리에서 뽑아온 [사용자의 테이블이라는 딕셔너리 뷰]
...
INSERT INTO MEMBER 
VALUES(   'MEM01','123ABC','김용가리','990809-1234567',DEFAULT );
...
-- 추가 샘플 데이터 삽입
INSERT INTO MEMBER VALUES(   'MEM02','456CDF','오리백숙','990809-9875447',SYSDATE );
COMMIT;
...
INSERT INTO MEMBER VALUES('MEM03','Q1W2E3','김치햄버거','990801-1124231',DEFAULT);
COMMIT;
SELECT * FROM MEMBER;

-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEAFULT 값이 반영됨
INSERT INTO MEMBER(MEMBER_ID,MEMBER_PWD,MEMBER_NAME,MEMBER_SSN) 
VALUES('MEM03','Q1W2E3','황의정승','990801-1124231');
...
INSERT INTO MEMBER2 VALUES('홍길동',998081234567);

INSERT INTO MEMBER2 VALUES('김영희',0011134567890);
SELECT * FROM MEMBER2;
--NUMBER TYPE 컬럼에 데이터 삽입 시 앞에 0이 있으면 사라지는 문제점이 있음
...
/*
    ...
    데이터 무결성 보장을 목적으로 함.
    -> 데이터 무결성이란?   중복 데이터 X  , NULL도 X 가장 이상적인 무결성
...
CREATE TABLE USER_USED_NN(
    USER_NO NUMBER NOT NULL,  --사용자 번호 (무조건 다들 있어야함. == NOT NULL) 
                                -- 컬럼레벨 제약 조건 설정
    USER_ID VARCHAR2(20) ,
...
-->모든 컬럼의 값이 같아야 중복으로 취급
-->복합키는 테이블 레벨로만 지정 가능하다.
...
INSERT INTO USER_USED_UK2
VALUES(2, 'user02', 'pass01', '김사장', '남', '010-1234-5678', 'hong123@kh.or.kr');
--> USER_ID가 다름
...
-- PRIMARY : 중요하다  주민번호 / 학번 /도서번호 /회원번호
--                 이 값 하나만 가지고 행의 모든 정보에 접근할 수 있다
--                 테이블에서 한행의 컬럼을 찾기위해 사용
...
CREATE TABLE USER_USED_PK(
    USER_NO NUMBER /*CONSTRAINT USER_NO_PK* 생략 가능 PRIMARY KEY */,  --컬럼 레벨로 PK 지정 
    ...
    CONSTRAINT USER_USERD_PK PRIMARY KEY (USER_NO)
);
...
--ORA-00001: 무결성 제약 조건(KHG.PK_USERNO_USERID)에 위배됩니다
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER, --CONSTRAINT GRADE_CODE_FK REFERENCES USER_GARDE(GRADE_CODE) --컬럼레벨로 FK 설정
  
  CONSTRAINT GRADE_CODE_FK FOREIGN KEY(GRADE_CODE) REFERENCES USER_GRADE
                                                                                            --참조 테이블 컬럼 생략 시 자동으로 PK를 참조
);
...
--ORA-02291: 무결성 제약조건(KHG.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
-- FK                       참조 무결성 위배 / 참조하는 곳에 값이 있어야 하는데 오류
...
SELECT * FROM USER_GRADE2;
SELECT * FROM USER_USED_FK2; --자식테이블도 같이 NULL으로 변했다.
...
SELECT * FROM USER_GRADE3; --부모행 삭제 (종속관계인행이 모드 삭제)
SELECT * FROM USER_USED_FK3;  -- 동시에 자식행도 삭제
...
CREATE TABLE USER_TEST(
    USER_NO VARCHAR2(20),
    USER_ID VARCHAR(20) CONSTRAINT PK_USER_ID UNIQUE,
    USER_PWD VARCHAR(20) CONSTRAINT NN_USER_PWD NOT NULL,
    PNO CHAR(14),
    GENDER CHAR(3) CONSTRAINT CK_GENDET CHECK (GENDER IN('남','여')),
    PHONE CHAR(13) ,
    ADDRESS VARCHAR(200),
    STATUS CHAR(1) CONSTRAINT NN_TAUSE NOT NULL ,CONSTRAINT CK_STATUS CHECK(STATUS IN('Y','N')),
...
-- 3) 테이블 형태( 컬럼명 , 데이터 타입)만 복사 (내부 데이터는 X)
...
CONSTRAINT FK_DEPT_CODE FOREIGN KEY(DEPT_CODE) REFERENCES DEPARTMENT;
                                                         --컬럼명 생략시 DEPARTMENT PK 컬럼과 자동 연결
