...
-- 사번, 이름, 직급, 부서코드, 급여, 입사일을 조회
 SELECT EMP_NAME,JOB_CODE,DEPT_CODE,SALARY,HIRE_DATE
 FROM EMPLOYEE
...
-- * SELECT 절에도 서브쿼리 사용할 수 있음
 ...
 CASE  WHEN EMP_ID IN(SELECT DISTINCT MANAGER_ID
                          FROM EMPLOYEE
                          WHERE MANAGER_ID IS NOT NULL) --매니저 아이디 일때 사수
                THEN '사수'
                ELSE '사원'
  END 구분
...
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME ='과장'
AND SALARY > ALL (SELECT SALARY FROM EMPLOYEE JOIN JOB USING(JOB_CODE) 
                    WHERE JOB_NAME ='차장');
...
-- EX 70~80년 사이의  남자 사원 중 
   
SELECT EMP_NAME
FROM EMPLOYEE
WHERE SUBSTR(EMP_NO,1,2)>='80' AND SUBSTR(EMP_NO,1,2) < '90';

SELECT EMP_NAME,JOB_NAME,DEPT_TITLE,SUBSTR(EMP_NO,1,2)||'년생' 년생
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON(DEPT_CODE=DEPT_ID)
JOIN  JOB USING(JOB_CODE)
...
SELECT EMP_ID,EMP_NAME,NVL(DEPT_TITLE,'소속없음'),JOB_NAME,TO_CHAR(HIRE_DATE,'YYYY-MM-DD'),DEPT_CODE
FROM EMPLOYEE E
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE)
WHERE ENT_YN = 'N' AND
...
--2 조회된 행 앞에 1부터 순서대로 증가하는 번호 붙이기.
...
--> SELECT 해석 순서를 고려하지 않아 올바르게 조회되지 않았다.
--> 이 문제를 해결하기 위해 인라인 뷰가 필요

--해결 1) 급여 내림차 조회 먼저
SELECT EMP_NAME,SALARY
FROM EMPLOYEE
ORDER BY SALARY DESC;
--해결 1의 조회 결과 (RESULTSET)을 [가상의 테이블(==VIEW)] 취급을 할 예정

--해결 2) 해결1의 조회 결과를 FROM 절에 사용한 후
--        상위 5행만 조회
SELECT ROWNUM , EMP_NAME,SALARY
FROM 
    (SELECT EMP_NAME,SALARY
...
SELECT NVL(DEPT_CODE,'부서없음'),NVL(DEPT_TITLE,'부서없음'),FLOOR(AVG(SALARY))
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
